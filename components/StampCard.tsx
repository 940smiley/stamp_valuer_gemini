...
const StampCard: React.FC<StampCardProps> = ({ stamp, collections, onRemove, onUpdate, onEditImage }) => {
  ...
  // Check privacy setting from local storage indirectly
  const settings = JSON.parse(localStorage.getItem('stamp-valuer-settings') || '{}');
  const isPrivate = settings.isPrivateCollection;
  const fbToken = settings.facebookAccessToken;
  ...
  const shareItem = async (platform: 'facebook' | 'twitter' | 'copy') => {
      const text = `Check out this rare ${localData.year} ${localData.country} stamp! Value: ${localData.estimatedValue}`;
      const url = window.location.href; 
      
      if (platform === 'facebook') {
          if (fbToken) {
              // Post directly to page/feed if connected
              if(window.confirm("Post this item to your connected Facebook Page?")) {
                  try {
                      await postToFacebook(text + "\n" + localData.description, undefined, fbToken);
            ...
            <div className="relative">
                ...
                {isShareOpen && (
                    <div className="absolute right-0 mt-2 w-32 bg-white rounded-lg shadow-xl border border-slate-100 py-1 z-20">
                        <button onClick={() => shareItem('facebook')} className="block w-full text-left px-4 py-2 text-xs hover:bg-slate-50 text-blue-700 font-bold">{fbToken ? 'Post to Page' : 'Facebook'}</button>
