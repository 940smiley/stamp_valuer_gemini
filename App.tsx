        ...
        <nav className="container mx-auto px-4 md:px-8 flex space-x-6 min-w-max">
            ...
            <div className="flex items-center ml-auto">
                <input 
                    ...
                    className="text-xs border rounded p-1.5 w-32 mr-2"
                    value={newCollectionName}
                    onChange={(e) => setNewCollectionName(e.target.value)}
                    onKeyDown={(e) => e.key === 'Enter' && handleAddCollection()}
                />
                <button onClick={handleAddCollection} className="text-xs bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold px-2 py-1.5 rounded">+</button>
            </div>
        </nav>
      </div>
          ...
          {activeView === 'dashboard' && (
            <div className="space-y-8">
              <div className="bg-white rounded-xl shadow-lg p-6 md:p-8 border border-slate-200">
                ...
                {videoResult && (
                    <div className="mt-6 bg-slate-50 border border-slate-200 rounded-xl p-6 animate-fade-in">
                        <div className="flex items-center gap-3 mb-4">
                             <div className="p-3 bg-purple-100 rounded-full text-purple-600">
                                <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                             </div>
                             <h3 className="text-xl font-bold text-slate-800">Collection Video Analysis</h3>
                        </div>
                             ...
                             <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
                                 ...
                                 <div className="flex flex-wrap gap-1 mt-1">
                                    {videoResult.countries.map((c: string, i: number) => (
                                        <span key={i} className="text-xs bg-slate-100 px-2 py-1 rounded">{c}</span>
                                    ))}
                                 </div>
                             </div>
                        </div>
                        ...
                        <div className="bg-white p-5 rounded-lg shadow-sm border border-slate-200">
                            ...
                            <p className="text-slate-600 leading-relaxed">{videoResult.summary}</p>
